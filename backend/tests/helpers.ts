...
/**
 * Make an admin request to the API using admin token header.
 */
export async function adminTokenApi(
  path: string,
  adminToken: string,
  options?: RequestInit
): Promise<Response> {
...
/**
 * Sign up a test admin user and return the token and user object.
 * Admin users have emails ending with @ngcb.nv.gov
 */
export async function signUpTestAdminUser(): Promise<TestUser> {
  const id = crypto.randomUUID();
  const res = await api("/api/auth/sign-up/email", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ...
      email: `testadmin+${id}@ngcb.nv.gov`,
      password: "TestPassword123!",
    }),
  });
  ...
  if (!res.ok) {
    const body = await res.text();
    throw new Error(`Failed to sign up test admin user (${res.status}): ${body}`);
  }
